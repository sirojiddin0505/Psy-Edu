...
function YakuniyTestPage() {
  ...
  const AddTest = async (e) => {
    ...
    try {
      ...
      {
        isEdit
          ? toast.error("Test o'zgartirishni imkoni bo'lmadi")
  ...
  const getTestCategory = async () => {
    try {
      const res = await axios.get(
        "https://testpsyedu.limsa.uz/tests/random-questions/category-type?category=last"
      );
            ...
            <tbody>
              {filteredTests.map((test, i) => (
                <tr key={test.id} className="bg-gray-600  ">
                  ...
                  <td className="text-white font-[700] text-[14px] border-2 border-gray-400 w-[250px] px-4 py-2">
                    {test.text}
                  </td>
                  <td
            ...
            >
              ...
              <form onSubmit={AddTest} className="flex flex-col gap-4">
                <input
                  ...
                  className="border border-gray-300 px-2 py-2 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
